...
const blockW = 60, blockH = 40, cols = 100, rows = 100;
...
function checkStatus() {
    ...
    db.ref('notifications/' + id).once('value', s => {
        const d = s.val();
        if(!d) alert("Order ID not found or still pending.");
...
viewport.addEventListener('wheel', (e) => { e.preventDefault(); scale = Math.min(Math.max(scale * (e.deltaY > 0 ? 0.9 : 1.1), 0.05), 6); updateUI(); }, { passive: false });
